...
A very simple tutorial, that will help you to discover Play Scala with the famous 'Hello World' example.
...
It will prompt you for the full application name. Type **Hello world**.
...
You can see that controller objects extend the **play.mvc.Controller** class. This class provides all useful methods for controllers. Also we have imported the @play.mvc._@ package that contains everything needed to write controllers.
...
In the @index@ action, we update the call to @html.index@ to pass no arguments, since a title is assigned by default. In the @sayHello@ action we retrieve the @myName@ parameter value coming from the form submission, and we pass it to the @html.sayHello@ template.
...
Now our **sayHello** can return 2 distinct values. An @Html@ value is returned in the case where there is a name provided by the form, and an @Action@ value is returned if there is an error. An @Action@ value is used to redirect to another existing action method. Here we specify that we want to redirect to the @index@ action.
...
We have now fully tested the application. Just select this test in the test runner and click ‘Start’. Everything should be green!
